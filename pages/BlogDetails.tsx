...
const BlogDetails: React.FC = () => {
    ...
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center text-center px-6 dark:bg-zinc-800">
        ...
        <button
          ...
          className="px-6 py-3 bg-primary-500 text-white rounded-full text-xs font-bold  tracking-normal shadow-lg hover:bg-zinc-800 transition-colors"
        >
          Back to Blog
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-800 pb-32">
      <SEO
        title={`${blog.seoTitle || blog.title} | VibeGadget`}
        description={blog.seoDescription || blog.excerpt}
        image={blog.metaImage || blog.image}
        jsonLd={jsonLd}
      />
      ...
      <div className="w-full h-[40vh] md:h-[60vh] relative overflow-hidden bg-zinc-900 dark:bg-zinc-50 dark:text-black flex items-end">
        ...
        <button
          onClick={() => navigate("/blog")}
          className="absolute top-8 left-6 md:left-12 z-20 w-10 h-10 flex items-center justify-center bg-zinc-50 dark:bg-zinc-900/40 backdrop-blur-md rounded-2xl text-zinc-900 dark:text-zinc-100 border border-zinc-200 dark:border-white/20 hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors shadow-lg active:scale-95"
        >
        ...
        <div className="relative z-10 p-6 md:p-12 md:pb-16 max-w-4xl w-full mx-auto">
          ...
          >
            <p className="text-[10px] text-white font-bold  tracking-normal mb-4 inline-block px-3 py-1 bg-zinc-900 dark:bg-white/40 backdrop-blur-sm rounded-full border border-white/20">
                ...
                a: ({ node, ...props }) => {
                  if (props.href?.startsWith("prod://")) {
                    ...
                    return (
                      <Link
                        to={`/product/${pid}`}
                        className="not-prose flex items-center gap-4 p-4 bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl hover:border-emerald-500 transition-colors shadow-sm my-6 cursor-pointer group no-underline"
                      >
