...
export default function CheckoutPage() {
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 2) {
      ...
      if (paymentType === "advance") return true; // Method selected
      return false;
    }
    if (step === 3) {
      if (paymentType === "cod") return true;
  ...
  const nextStep = () => {
    if (validateStep(currentStep)) {
      setCurrentStep((p) => {
        if (p === 2 && paymentType === "cod") return 4;
  ...
  const prevStep = () => setCurrentStep((p) => {
    if (p === 4 && paymentType === "cod") return 2;
      ...
      <div className="max-w-7xl mx-auto p-6 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-start gap-4 flex-col">
            ...
            <div className="flex flex-col gap-2">
              <h1 className="text-2xl font-bold flex items-center gap-2 text-zinc-900 dark:text-zinc-100">Checkout</h1>
        ...
        <div className="flex items-center justify-between sm:justify-start sm:gap-6 py-4 overflow-x-auto no-scrollbar mask-linear-fade pr-4">
          {[
            ...
            { step: 2, label: "Method", icon: CreditCard },
            ...
            <div key={step} className="flex items-center gap-2 shrink-0">
              ...
              {index < 3 && <div className={cn("w-4 sm:w-8 h-0.5", currentStep > step ? "bg-emerald-500" : "bg-zinc-200 dark:bg-zinc-800")} />}
            </div>
          ))}
        </div>
            ...
            {currentStep === 2 && (
              <Card className="rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   ...
                   {paymentType === "advance" && (
                     <div className="flex flex-col gap-6 mt-4 border-t border-zinc-100 dark:border-zinc-800 pt-6 animate-in fade-in slide-in-from-top-4">
                       <p className="text-sm font-semibold text-zinc-600 dark:text-zinc-400">You selected Mobile Banking. Click Continue to complete payment.</p>
                     </div>
                   )}
                ...
                <CardFooter className="flex justify-between items-center border-t border-zinc-100 dark:border-zinc-800 pt-4 pb-2">
                  ...
                  <Button onClick={nextStep} disabled={!validateStep(2)} size="default" className="text-xs sm:text-sm px-4 sm:px-6">{paymentType === "cod" ? "Review Order" : "Continue to Payment"}</Button>
                </CardFooter>
              </Card>
            )}

            {/* Step 3: Payment Verification */}
            {currentStep === 3 && paymentType === "advance" && (
              <Card className="rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex flex-col gap-2">
                <CardHeader>
                  <h2 className="text-xl font-bold flex items-center gap-2 text-zinc-900 dark:text-zinc-100">
                ...
                <CardFooter className="flex justify-between items-center border-t border-zinc-100 dark:border-zinc-800 pt-4 pb-2">
                  <Button variant="ghost" className="text-xs sm:text-sm px-2 sm:px-4" onClick={prevStep}><ChevronLeft className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> Back</Button>
                  <Button onClick={nextStep} disabled={!validateStep(3)} size="default" className="text-xs sm:text-sm px-4 sm:px-6">Review Order</Button>
                </CardFooter>
              </Card>
            )}
            ...
            {currentStep === 4 && (
              <Card className="rounded-3xl shadow-sm border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex flex-col gap-2">
                ...
                <CardFooter className="flex flex-row justify-between items-center border-t border-zinc-100 dark:border-zinc-800 pt-4 pb-2 sm:gap-4 gap-2">
                  <Button variant="ghost" className="text-xs sm:text-sm px-2 sm:px-4" onClick={prevStep}><ChevronLeft className="mr-1 h-3 w-3 sm:h-4 sm:w-4" /> Back</Button>
                  <Button onClick={placeOrder} disabled={!validateStep(4) || isLoading} size="default" className="bg-emerald-500 hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 text-white border-0 text-xs sm:text-sm px-4 sm:px-6 flex-1 sm:flex-none">
